...
public class AhaManager {
	...
	public AhaManager(S3Client s3, String rootBucketName, SqsClient sqs) {
		this.s3 = s3;
	...
	/*
	 * public static void main(String[] args) { Region region = Region.EU_CENTRAL_1;
	 * String rootBucketName = "apuseni.eu-dev"; S3Client s3 =
	 * S3Client.builder().region(region).build(); SqsClient sqs =
	 * SqsClient.builder().region(region).build(); // SqsReader reader = new
	 * SqsReader(sqs, S3Storage.PROBLEM_VALIDATION_QUEUE); // new
	 * Thread(reader).start(); AhaManager s3st = new AhaManager(s3, rootBucketName,
	 * sqs); // // System.out.println(s3st.createContest("One more", "Romi")); // if
	 * (s3st.contestExists(5)) { // s3st.addProblem(5, "b"); // s3st.addProblem(5,
	 * "c"); // s3st.addProblem(5, "d"); // }
